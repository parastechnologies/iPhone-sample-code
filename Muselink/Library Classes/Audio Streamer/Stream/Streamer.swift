...
class Streamer: Streaming {
    ...
    func attachNodes() {
        ...
        if isProcessAudio {
            engine.mainMixerNode.installTap(onBus: 0, bufferSize: AVAudioFrameCount(readFormat.sampleRate), format: readFormat) { [weak self](buffer, time) in
