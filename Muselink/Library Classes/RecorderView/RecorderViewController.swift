...
class RecorderViewController: UIViewController {
    ...
    var settings = [AVFormatIDKey: kAudioFormatLinearPCM, AVLinearPCMBitDepthKey: 16, AVLinearPCMIsFloatKey: true, AVSampleRateKey:AVAudioSession.sharedInstance().sampleRate, AVNumberOfChannelsKey: 1] as [String : Any]
    ...
    fileprivate func setupHandelView() {
        ...
        handleView.backgroundColor = UIColor(r: 208, g: 207, b: 205,a:1)
    ...
    private func startRecording() {
        ...
        settings.updateValue(AVAudioSession.sharedInstance().sampleRate, forKey: AVSampleRateKey)
    ...
    private func stopRecording() {
        if let d = self.delegate {
            if let audURL = audioURL {
                navigationController?.dismiss(animated: true, completion: nil)
                d.didFinishRecording(audioURL: audURL)
            }
        }
    ...
    private func createAudioRecordPath() -> URL? {
        let currentFileName = "recordingUpload" + ".wav"
        guard let documentsDirectoryURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
            return nil
        }
        // check if the file already exist at the destination folder if you don't want to download it twice
        if FileManager.default.fileExists(atPath:  documentsDirectoryURL.appendingPathComponent(currentFileName).path) {
            try? FileManager.default.removeItem(atPath:  documentsDirectoryURL.appendingPathComponent(currentFileName).path)
        }
        return documentsDirectoryURL.appendingPathComponent(currentFileName)
    }
