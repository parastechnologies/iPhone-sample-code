...
class SignUpEmail_PhoneVC: BaseClassVC {
    ...
    private var passwordLabel : BindingTextField!{
        didSet {
            passwordLabel.bind{[unowned self] in self.viewModel.password.value = $0 }
        }
    }
    private var phoneLabel    : BindingTextField!{
        didSet {
            phoneLabel.bind{[unowned self] in self.viewModel.phone.value = $0 }
        }
    }
    ...
    var viewModel : RegistrationViewModel! {
        didSet { setUpVM(model: viewModel) }
    }
    override func viewDidLoad() {
        super.viewDidLoad()
        viewModel = RegistrationViewModel(type: .SignUp(type: .Phone))
    ...
    @IBAction func action_Email_Phone_Segment(_ sender: CustomSegmentedControl) {
        if sender.selectedSegmentIndex == 0 {
           isPhoneSignUp = true
            ...
            view_EmailSignUpBack.isHidden = false
            view_PhoneSignUpBack.isHidden = true
            viewModel = RegistrationViewModel(type: .SignUp(type: .Email))
        }
    }
